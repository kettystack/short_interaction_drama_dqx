...
用法：
    cd backend && PYTHONPATH=. python -m app.scripts.seed_tianxiadyi
"""
...
def episode_id(no: int) -> str:
    return f"txy_{no:03d}"


async def seed(video_root: Path, xlsx_path: Path, import_danmaku: bool) -> None:
    await init_db()
    files = sorted(video_root.glob("第*.mp4"), key=lambda item: int(EP_RE.search(item.name).group(1)) if EP_RE.search(item.name) else 0)
    async with SessionLocal() as db:
        for path in files:
            ...
            if item is None:
                item = Episode(
                    ...
                    episode_no=no,
                    video_url=f"/videos/tianxiadyi/{path.name}",
                )
                db.add(item)
            item.duration = probe_duration(path)
            item.video_url = f"/videos/tianxiadyi/{path.name}"
        ...
        if not xlsx_path.is_file():
            raise FileNotFoundError(f"danmaku xlsx not found: {xlsx_path}")

        ids = [episode_id(no) for no in range(1, 6)]
        await db.execute(delete(DanmakuItem).where(DanmakuItem.episode_id.in_(ids), DanmakuItem.source == "xlsx"))

        count = 0
        for row_index, row in enumerate(iter_danmaku_rows(xlsx_path)):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Seed Tianxiadyi episodes and danmaku.")
    parser.add_argument("--video-root", default=settings.tianxiadyi_video_root)
    parser.add_argument("--xlsx", default="../../前五集弹幕.xlsx")
    parser.add_argument("--skip-danmaku", action="store_true")
    args = parser.parse_args()
    asyncio.run(seed(Path(args.video_root).resolve(), Path(args.xlsx).resolve(), not args.skip_danmaku))
